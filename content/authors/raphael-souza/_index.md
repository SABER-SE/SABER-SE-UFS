---
title: Raphael Gomes Souza
first_name: Raphael
last_name: Gomes Souza

superuser: false

role: Doutorando

organizations:
  - name: Universidade Federal de Sergipe (UFS)
    url: 'https://www.ufs.br'

bio: Doutorando em Física investigando a evolução química da Galáxia e populações estelares.

interests:
  - Evolução Química da Galáxia
  - Populações Estelares
  - Espectroscopia de Alta Resolução
  - APOGEE

education:
  courses:
    - course: Mestrado em Física
      institution: Universidade Federal de Sergipe (UFS)
      year: 2022
    - course: Bacharelado em Física
      institution: Universidade Federal de Sergipe (UFS)
      year: 2020

social:
  - icon: envelope
    icon_pack: fas
    link: 'mailto:raphael.souza@academico.ufs.br'

  - icon: graduation-cap
    icon_pack: fas
    link: 'http://lattes.cnpq.br/PREENCHER'
    label: Currículo Lattes
email: 'raphael.souza@academico.ufs.br'

highlight_name: false

user_groups:
  - Doutorandos
---

**Raphael Gomes Souza** é doutorando em Física na UFS. Sua pesquisa no Grupo SABER-SE está centrada na evolução química da Via Láctea, utilizando abundâncias químicas derivadas de dados espectroscópicos do levantamento APOGEE para rastrear a história de enriquecimento químico de diferentes populações estelares.
